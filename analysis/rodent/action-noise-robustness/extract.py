...
One eval artifact per (run, sigma), each covering all three datasets::
    ...
    python -m vnl_experiments.artifacts plan --kind eval \\
        --runs analysis/rodent/action-noise-robustness/runs.csv \\
...
Run it
------
    ../.venv/bin/python analysis/rodent/action-noise-robustness/extract.py           # frozen
    ../.venv/bin/python analysis/rodent/action-noise-robustness/extract.py --refresh
    ../.venv/bin/python analysis/rodent/action-noise-robustness/extract.py --check
...
#: ``EXPECTED_STEP``, following ``analysis/rodent/collision-model-xml``. Completing all 600 M
