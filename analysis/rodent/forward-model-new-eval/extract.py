"""Repeat the explicit-forward-model comparison on the new eval set.

Run from the repo root::

    ../.venv/bin/python analysis/rodent/forward-model-new-eval/extract.py

Does the explicit forward model improve performance over the standard with-efference decoder,
now measured on the new batch-evaluation datasets? This re-uses the same three-dataset eval
(``train`` / ``old_eval`` / ``new_eval``) as ``train-eval-generalization`` but for three
conditions:

  - ``forward_model``  — RodentForwardModel, canonical ``fm_loss_weight == 1`` delay sweep.
  - ``efference``      — the standard with-efference EncDec decoder (the reference).
  - ``no_efference``   — no efference copy (floor).

It joins THREE sources (extract.py is the only stage that touches them):
  1. committed ``analysis/*/data.csv`` — authoritative run set + ``condition`` labels;
  2. WandB — independent re-verification of comparability invariants;
  3. local ``eval_results/*.json`` — the three-dataset metrics.

Output is a **long** ``data.csv`` (one row per run × dataset). Cumulative ``episode_reward`` is
NOT comparable across datasets (new clips are 30 s / 3002-step rollouts vs 5 s / 502-step), so
we emit length-normalised metrics (``reward_per_step``, ``survival_frac``) plus per-step error
and per-episode termination breakdowns, which ARE comparable.
"""
...
# Conditions kept for this analysis.
INCLUDE_CONDITIONS = {"forward_model", "efference", "no_efference"}
CANONICAL_FM_LOSS_WEIGHT = 1.0  # keep only the standard FM delay sweep (excl. loss-weight sweep)
...
def eval_rows(wid: str, ej: dict, base: dict) -> list[dict]:
    ...
    for ds in DATASETS:
        ...
        row.update({
            ...
            # Per-second failure hazard, excluding end-of-clip truncations: constant-hazard
            # MLE = events / exposure = (1 - survived) / total-alive-time. Length-independent,
            # so it compares fairly across the 5 s and 30 s clips. survived counts only failure
            # terminations (verified: survived + failure-reasons == 1), so truncations don't
            # count as events; surviving clips contribute their full lifetime as censored exposure.
            "hazard_rate": (
...
def main() -> None:
    ...
    for wid, info in cond_map.items():
        ...
        # Keep only the canonical FM delay sweep (drop the loss-weight sweep & weight-0 runs).
        if info["condition"] == "forward_model" and ej.get("fm_loss_weight") != CANONICAL_FM_LOSS_WEIGHT:
            continue
        base = dict(info)
