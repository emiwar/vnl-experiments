"""Tests for applying overrides onto an ml_collections config.
...
Overrides now arrive from two directions -- Hydra config groups as a nested tree, and
``KEY=VALUE`` strings -- and both go through the same validation, so both are covered
here.
"""
...
from vnl_experiments.config.overrides import (
    OverrideError,
    apply_strings as apply_env_config_overrides,
    apply_tree,
    coerce as _coerce,
    flatten,
)
...
class ApplyEnvConfigOverridesTest(absltest.TestCase):
    ...
    def test_unknown_top_level_key(self):
        with self.assertRaises(OverrideError) as e:
...
class TreeOverrideTest(absltest.TestCase):
    """The Hydra path: a nested mapping of already-typed values.

    Same validation as the string path, but no coercion step -- YAML has already produced
    an int, a float or a bool, and re-parsing it through a string would be a chance to get
    it wrong.
    """

    def test_nested_tree_is_flattened_and_applied(self):
        cfg = _config()
        apply_tree(cfg, {"ctrl_dt": 0.02,
                         "reward_terms": {"joints": {"weight": 0.5, "exp_scale": 2.0}}})
        self.assertEqual(cfg.ctrl_dt, 0.02)
        self.assertEqual(cfg.reward_terms.joints.weight, 0.5)
        self.assertEqual(cfg.reward_terms.joints.exp_scale, 2.0)

    def test_yaml_types_are_used_as_given(self):
        """A bool from YAML stays a bool; it is not round-tripped through a string."""
        cfg = _config()
        apply_tree(cfg, {"torque_actuators": False, "njmax": 512})
        self.assertIs(cfg.torque_actuators, False)
        self.assertEqual(cfg.njmax, 512)

    def test_unknown_key_in_a_tree_is_refused(self):
        with self.assertRaises(OverrideError) as e:
            apply_tree(_config(), {"reward_terms": {"jointz": {"weight": 1.0}}})
        self.assertIn("jointz", str(e.exception))

    def test_list_value_is_one_leaf_not_indexed_keys(self):
        cfg = _config()
        apply_tree(cfg, {"start_frame_range": [0, 120]})
        self.assertEqual(list(cfg.start_frame_range), [0, 120])

    def test_flatten_matches_dotted_form(self):
        self.assertEqual(
            flatten({"a": {"b": 1, "c": {"d": 2}}, "e": [1, 2]}),
            {"a.b": 1, "a.c.d": 2, "e": [1, 2]},
        )
