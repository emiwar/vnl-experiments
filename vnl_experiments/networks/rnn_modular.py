...
class RNNModularNetwork(PPONetwork):
    ...
    def __init__(self,
        ...
        self.self_connections = nnx.Dict({k: nnx.Linear(ms, ms, rngs=rngs) for k,ms in module_size.items()})
    ...
    def __call__(self,
        ...
        new_state = {k: module_input[k] + self.self_connections[k](h[k]) for k in module_input.keys()}
    ...
    def initialize_state(self, batch_size: int) -> tuple[()]:
        state = {k: jp.zeros((batch_size, sc.in_features)) for k,sc in self.self_connections.items()}
        if self.detached_critic:
    ...
    def reset_state(self, prev_state):
        new_state = dict()
        for k in self.self_connections.keys():
